...
public class Repnet extends Contract {
    ...
    public RateEventEventResponse getRateEventEvent(Log log) {
        final Event event = new Event("RateEvent",
                Arrays.<TypeReference<?>>asList(),
                Arrays.<TypeReference<?>>asList(new TypeReference<Address>() {}, new TypeReference<Address>() {}, new TypeReference<Uint8>() {}));
        EventValuesWithLog value = extractEventParametersWithLog(event, log);
        RateEventEventResponse typedResponse = new RateEventEventResponse();
        typedResponse.log = value.getLog();
        typedResponse.sender = (String) value.getNonIndexedValues().get(0).getValue();
        typedResponse.receipient = (String) value.getNonIndexedValues().get(1).getValue();
        typedResponse.score = (BigInteger) value.getNonIndexedValues().get(2).getValue();
    ...
    public Subscription subscribeRateEventEventObservable(DefaultBlockParameter startBlock, final Observer<RateEventEventResponse> observer) {
        final Event event = new Event("RateEvent",
                Arrays.<TypeReference<?>>asList(),
                Arrays.<TypeReference<?>>asList(new TypeReference<Address>() {}, new TypeReference<Address>() {}, new TypeReference<Uint8>() {}));
        EthFilter filter = new EthFilter(startBlock, DefaultBlockParameterName.PENDING, getContractAddress());
        filter.addSingleTopic(EventEncoder.encode(event));
        return web3j.ethLogObservable(filter).subscribe(new Observer<Log>() {
            @Override
